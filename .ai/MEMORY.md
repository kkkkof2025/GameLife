...
### 2026-05-25 - Playwright 自动化固化
1. 新增 `scripts/run_smoke.py`，使用 Playwright 路由拦截模拟同源 HTTP 环境，不依赖本地端口。
2. 新增 `.github/workflows/smoke.yml`，push/PR 到 `main` 时自动安装 Playwright 并运行 smoke。
3. 后续修改多人联机流程时，优先运行 `python scripts/run_smoke.py`，确认创建、加入、断线重连、赛后结果和最终成绩闭环不回退。

### 2026-05-26 - 多人房间历史
1. 新增 `lifecash_room_history` 本地房间历史，记录多人结算后的房间状态、最终排名、个人名次、财富和分数。
2. 排行榜入口旁新增“房间历史”弹窗，支持查看最近对局快照和清空本地历史；排行榜导出/Gist 同步数据也包含 `roomHistory`。
3. `smoke-test.html` 补充房间历史写入和弹窗校验，`python scripts/run_smoke.py` 已覆盖创建、加入、断线重连、赛后结果、房间历史和最终成绩。
